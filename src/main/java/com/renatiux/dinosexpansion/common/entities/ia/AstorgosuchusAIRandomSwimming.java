...
public class AstorgosuchusAIRandomSwimming extends RandomWalkingGoal {

    public AstorgosuchusAIRandomSwimming(CreatureEntity creature, double speed, int chance) {
        super(creature, speed, chance, false);
    }

    public boolean shouldExecute() {
        if (this.creature.isBeingRidden() || ((Astorgosuchus)creature).isSitting() || creature.getAttackTarget() != null || !this.creature.isInWater() && this.creature instanceof ISemiAquatic && !((ISemiAquatic) this.creature).shouldEnterWater()) {
            return false;
        } else {
            if (!this.mustUpdate) {
