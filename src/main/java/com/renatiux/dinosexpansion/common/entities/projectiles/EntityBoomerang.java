...
public abstract class EntityBoomerang extends Entity{
    ...
    public EntityBoomerang(EntityType<? extends EntityBoomerang> type, World world) {
        super(type, world);
    ...
    public EntityBoomerang(EntityType<? extends EntityBoomerang> type, World worldIn, PlayerEntity entity, ItemStack itemstack, Hand hand) {
        this(type, worldIn);
    ...
    public void tick() {
        ...
        RayTraceResult raytraceresult = this.world.rayTraceBlocks(new RayTraceContext(vec3d1, vec3d, BlockMode.OUTLINE, FluidMode.ANY, (Entity)null));
        if (raytraceresult != null && raytraceresult.getType() == RayTraceResult.Type.BLOCK) {
            BlockPos pos = new BlockPos(raytraceresult.getHitVec());
            BlockState state = this.world.getBlockState(pos);
            if (state.getMaterial() == Material.PLANTS && DEModConfig.BOOMERANGCONFIG.breaksFlowers.get()) {
                this.world.destroyBlock(pos, true);
            }
 
            if (state.getMaterial() == Material.TALL_PLANTS && DEModConfig.BOOMERANGCONFIG.breaksTallGrass.get()) {
                this.world.destroyBlock(pos, true);
            }
 
            if (state.getBlock() instanceof TorchBlock && DEModConfig.BOOMERANGCONFIG.breaksTorches.get()) {
                this.world.destroyBlock(pos, true);
            }
 
            if (state.getBlock() instanceof LeverBlock && DEModConfig.BOOMERANGCONFIG.activatesLevers.get()) {
                if (this.timeBeforeTurnAround > 0 && DEModConfig.BOOMERANGCONFIG.turnAroundButton.get()) {
            ...
            if (state.getBlock() instanceof AbstractButtonBlock && DEModConfig.BOOMERANGCONFIG.activatesButtons.get()) {
                if (this.timeBeforeTurnAround > 0 && DEModConfig.BOOMERANGCONFIG.turnAroundButton.get()) {
                    this.timeBeforeTurnAround = 0;
                }
            ...
            if (state.getBlock() instanceof PressurePlateBlock && DEModConfig.BOOMERANGCONFIG.activatesPressurePlates.get()) {
                if (this.timeBeforeTurnAround > 0 && DEModConfig.BOOMERANGCONFIG.turnAroundButton.get()) {
                    this.timeBeforeTurnAround = 0;
                }
            ...
            if (state.getBlock() instanceof TripWireBlock && DEModConfig.BOOMERANGCONFIG.activatesTripWire.get()) {
                if (this.timeBeforeTurnAround > 0 && DEModConfig.BOOMERANGCONFIG.turnAroundButton.get()) {
                    this.timeBeforeTurnAround = 0;
                }
        ...
        for(int i = 0; i < list.size(); ++i) {
            ...
            if (entity instanceof ItemEntity) {
                ...
                if (this.timeBeforeTurnAround > 0 && DEModConfig.BOOMERANGCONFIG.turnAroundItem.get()) {
                    this.timeBeforeTurnAround = 0;
                }
            } else if (entity instanceof LivingEntity && entity != player) {
                this.onEntityHit(entity, player);
                if (this.timeBeforeTurnAround > 0 && DEModConfig.BOOMERANGCONFIG.turnAroundMob.get()) {
                    this.timeBeforeTurnAround = 0;
                }
            }
        }
 
        Iterator<ItemEntity> iterator = this.itemsPickedUp.iterator();
    ...
    public abstract DamageSource causeNewDamage(EntityBoomerang var1, Entity var2);
    ...
    static {
        ...
        RETURN_UNIQUE_ID = EntityDataManager.createKey(EntityBoomerang.class, DataSerializers.OPTIONAL_UNIQUE_ID);
    }
