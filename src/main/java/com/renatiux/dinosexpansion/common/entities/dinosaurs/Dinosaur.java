...
public abstract class Dinosaur extends MonsterEntity
    ...
    public static final DataParameter<Byte> LEVEL_PROGRESS = EntityDataManager.createKey(Dinosaur.class, DataSerializers.BYTE);
    ...
    protected int sleepCooldown, hungerCounter, growingAge, poopCooldown, breedCooldown, tamingNumber, levelCounter;
    ...
    public void addTamingProgress(int tamingProgress) {
        this.tamingNumber += tamingProgress;
        this.dataManager.set(TAMING_PROGRESS, (byte) ((double) (this.tamingNumber) * 100d / (double) (neededHungerToTame())));
    }
    ...
    protected void levelUp() {
        ...
        this.levelCounter = 0;
        this.getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue(this.getAttribute(Attributes.ATTACK_DAMAGE).getBaseValue() + 0.5d);
        this.getAttribute(Attributes.MAX_HEALTH).setBaseValue(this.getAttribute(Attributes.MAX_HEALTH).getBaseValue() + 1);
    }
     ...
     */
    public void findAndAddHunger(boolean taming) {
        for (int i = 0; i < tamingInventory.getSizeInventory(); i++) {
            if (canEat(tamingInventory.getStackInSlot(i))
                    || Tags.Items.KIBBLE.contains(tamingInventory.getStackInSlot(i).getItem())) {
                int hungerToAdd = tamingInventory.getStackInSlot(i).getItem().getFood().getHealing();
                if (hungerToAdd <= this.getMaxHunger() - this.getHunger()) {
                    addHunger(hungerToAdd);
                    if (taming) {
                        addTamingProgress(hungerToAdd);
                    }
    ...
    public double getPercentHunger() {
        return (double) (getHunger()) / (double) (getMaxHunger());
    }
    ...
    protected void registerData() {
        ...
        this.dataManager.register(HUNGER, getMaxHunger());
    ...
    public void readAdditional(CompoundNBT compound) {
        ...
        if (deathTime > 0) {
            for (int i = 0; compound.contains("stack" + i); i++) {
                this.stacksToDrop.add(ItemStack.read(compound.getCompound("stack" + i)));
            }
            return;
        }
    ...
    public void writeAdditional(CompoundNBT compound) {
        ...
        if (deathTime > 0) {
            ...
            for (ItemStack stack : this.stacksToDrop) {
                compound.put("stack" + i, stack.write(compound));
            }
            return;
        }
        ...
        compound.putString("sex", this.getSex().name());
        compound.putInt("level", this.getLevel());
        compound.putInt("levelPoints", this.levelCounter);
        compound.putByte("tamingProgress", this.getTamingProgress());
    ...
    /**
     * @return determines how many times the entity has to eat before it can be tamed
     */
    public abstract int neededHungerToTame();

    /**
     * defines teh Tier of the dino
     * lower Tier dinos will never attack higher Tier dinos by themselves except when tamed and said to
     *
     * @return - the tier of the Dino
     */
    public abstract int getTier();

    /**
     * make the dino hungry
     * called every tick
     */
    public abstract int reduceHunger(int hunger);

    public boolean isHungry() {
        return getPercentHunger() < 0.7;
    }
    ...
    public byte getLevelProgress() {
        return this.dataManager.get(LEVEL_PROGRESS);
    }

    protected void addLevelPoints(int points) {
        this.levelCounter += points;
        this.dataManager.set(LEVEL_PROGRESS, (byte) ((double) (levelCounter) * 100d / (double) (this.getMaxRequiredLevelPoints())));
    }
    ...
    protected int getMaxRequiredLevelPoints() {
        int baseValue = DEModConfig.DINO_CONFIG.baseNeededLevelToLevelUp.get();
        int requiredLevelPoints = baseValue * getTier() + 20 * this.getLevel();
    ...
    protected int getInitialLevel() {
        return rand.nextInt(DEModConfig.DINO_CONFIG.maxLevelSpawn.get() - DEModConfig.DINO_CONFIG.minLevelSpawn.get()) + DEModConfig.DINO_CONFIG.minLevelSpawn.get();
    }
    ...
    protected void setSex(Sex sex) {
        this.dataManager.set(SEX, sex.name());
    }

    /**
     * @return the taming progress always goes from 0 to 100
     */
