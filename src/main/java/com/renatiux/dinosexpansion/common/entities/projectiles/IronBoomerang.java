...
public class IronBoomerang extends EntityBoomerang{
	...
	public IronBoomerang(EntityType<? extends EntityBoomerang> type, World world) {
		super(type, world);
		this.timeBeforeTurnAround = DEModConfig.BOOMERANGCONFIG.IronBoomerangRange.get() <= 0 ? 20 : DEModConfig.BOOMERANGCONFIG.IronBoomerangRange.get();
	}
	...
	protected int getDamage(Entity var1, PlayerEntity var2) {
		return DEModConfig.BOOMERANGCONFIG.IronBoomerangDamage.get() > 0 ? DEModConfig.BOOMERANGCONFIG.IronBoomerangDamage.get() : 0;
	}
	...
	public DamageSource causeNewDamage(EntityBoomerang var1, Entity var2) {
	 ...
	 @Override
	    public void beforeTurnAround(PlayerEntity player) {
	        if (!isBouncing && DEModConfig.BOOMERANGCONFIG.IronBoomerangFollows.get()) {
	...
	public ItemStack getRenderedItemStack() {
		return new ItemStack(ItemInit.IRON_BOOMERANG.get());
	}
