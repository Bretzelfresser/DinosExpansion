...
public class Charnia extends LivingEntity implements IAnimatable {
    ...
    public Charnia(EntityType<? extends LivingEntity> type, World worldIn) {
        super(type, worldIn);
    }

    private PlayState predicate(AnimationEvent<Charnia> event) {
        if (event.getController().getAnimationState() == AnimationState.Stopped)
            event.getController()
                    .setAnimation(new AnimationBuilder().addAnimation("animation.Charnia.Movement1", true));
    ...
    public void registerControllers(AnimationData data) {
        data.addAnimationController(new AnimationController<Charnia>(this, CONTROLLER_NAME, 10, this::predicate));
    }
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 9) {
            GeckoLibUtil.getControllerForID(factory, getEntityId(), CONTROLLER_NAME).setAnimation(
                    new AnimationBuilder().addAnimation("animation.Charnia.MovementTurbulent", false));
        }
