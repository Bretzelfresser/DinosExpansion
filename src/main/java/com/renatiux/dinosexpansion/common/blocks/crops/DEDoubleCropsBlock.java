...
public class DEDoubleCropsBlock extends DECropsBlock{

    public static final EnumProperty<DoubleBlockHalf> HALF = BlockStateProperties.DOUBLE_BLOCK_HALF;

    public DEDoubleCropsBlock(Block.Properties properties,  Supplier<Item> seedItemSupplier) {
        super(properties, seedItemSupplier);
    ...
    public void grow(World worldIn, BlockPos pos, BlockState state) {
        ...
        if (newAge >= maxAge - 1 && worldIn.getBlockState(pos.up()) == Blocks.AIR.getDefaultState() && worldIn.getBlockState(pos.down()).getBlock() instanceof FarmlandBlock) {
            ...
            worldIn.setBlockState(pos.up(), withAge(maxAge).with(DEDoubleCropsBlock.HALF, DoubleBlockHalf.UPPER), Constants.BlockFlags.BLOCK_UPDATE);
            return;
        }
    ...
    public void onBlockHarvested(World worldIn, BlockPos pos, BlockState state, PlayerEntity player) {
        if (!worldIn.isRemote) {
            ...
            if (state.getBlock() == this && below == this) { //make sure the block below is also a double crop block
                DEDoubleCropsBlock crop = (DEDoubleCropsBlock) worldIn.getBlockState(pos).getBlock();
