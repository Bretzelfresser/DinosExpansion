...
public class AllosaurusEggBlock extends Block {
    ...
    private boolean canTrample(World worldIn, Entity trampler) {
        if (trampler instanceof Allosaurus) {
            return false;
        } else {
            return !(trampler instanceof LivingEntity) || trampler instanceof PlayerEntity || net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(worldIn, trampler);
        }
    }
}
