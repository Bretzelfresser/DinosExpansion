...
public abstract class BaseDinosaurEgg extends Block implements IIncubatorEgg{
	...
	private boolean hasProperHabitat(IBlockReader blockReader, BlockPos pos) {
		return blockReader.getBlockState(pos.down()).getBlock() == Blocks.SAND
				...
				|| blockReader.getBlockState(pos.down()).getBlock() == Blocks.GRASS_BLOCK
				|| blockReader.getBlockState(pos.down()).getBlock() == Blocks.DIRT
				|| blockReader.getBlockState(pos.down()).getBlock() == Blocks.PODZOL;
	}
	...
	public boolean isReplaceable(BlockState state, BlockItemUseContext useContext) {
		return useContext.getItem().getItem() == this.asItem() && state.get(EGGS) < 4
	...
	private boolean canTrample(World worldIn, Entity trampler) {
		for (Class<Dinosaur> clazz : canTrampleOn()) {
