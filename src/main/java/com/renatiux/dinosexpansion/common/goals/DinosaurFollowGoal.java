...
public class DinosaurFollowGoal extends Goal {
	private final Dinosaur tameable;
	...
	public DinosaurFollowGoal(Dinosaur tameable, double speed, float minDist, float maxDist) {
		...
		this.teleportToLeaves = false;
		this.setMutexFlags(EnumSet.of(Goal.Flag.MOVE, Goal.Flag.LOOK));
		if (!(tameable.getNavigator() instanceof GroundPathNavigator)
				&& !(tameable.getNavigator() instanceof FlyingPathNavigator)) {
			throw new IllegalArgumentException("Unsupported mob type for DinosaurFollowGoal");
		}
	}
	...
	public boolean shouldExecute() {
		...
		if (livingentity == null) {
			...
			return false;
		} else if(tameable.getStatus() == DinosaurStatus.FOLLOW && !tameable.isSleeping() && !tameable.isKnockout() && tameable.deathTime <= 0){
	...
	private boolean tryToTeleportToLocation(int x, int y, int z) {
		if (Math.abs((double) x - this.owner.getPosX()) < 2.0D && Math.abs((double) z - this.owner.getPosZ()) < 2.0D) {
			...
			this.tameable.setLocationAndAngles((double) x + 0.5D, (double) y, (double) z + 0.5D,
